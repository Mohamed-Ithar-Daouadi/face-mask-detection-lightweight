# 03_train_yolov8n.py
# Trains YOLOv8n on the face mask detection dataset
...
IMG_SIZE   = 640          # standard YOLOv8 input size (matches pretrained weights)
BATCH      = 16           # community standard for YOLOv8 fine-tuning on custom datasets
DEVICE     = 0            # force GPU — change to 'cpu' if running locally
PROJECT    = "results"    # output folder
NAME       = "yolov8n"    # subfolder inside results/
# ──────────────────────────────────────────────────────────────────────────────

def main():
    # Load pretrained YOLOv8n weights (transfer learning from COCO)
    model = YOLO("yolov8n.pt")

    print("=" * 60)
    print("Training YOLOv8n — Face Mask Detection")
    ...
    print("\n✅ Training complete!")
    print(f"   Best weights saved to: results/yolov8n/weights/best.pt")
